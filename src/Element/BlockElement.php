...
use League\CommonMark\InlineParser;
...
class BlockElement
{
    ...
    public function __construct($type, $startLine, $startColumn, $endLine = null)
    {
        ...
        $this->inlineContent = new ArrayCollection();
    }
     ...
     * @return $this
     */
    public function setParent(BlockElement $parent)
    {
        $this->parent = $parent;
     ...
     */
    public function finalize($lineNumber, InlineParser $inlineParser, ReferenceMap $refMap)
    {
        if (!$this->open) {
            return;
        }
        ...
        switch ($this->getType()) {
            ...
            case self::TYPE_FENCED_CODE:
                ...
                $this->setExtra('info', RegexHelper::unescape(trim($this->strings->first())));
