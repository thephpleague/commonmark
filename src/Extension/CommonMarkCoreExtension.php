...
final class CommonMarkCoreExtension implements ExtensionInterface
{
    ...
    {
        $environment
            ->addBlockParser(new BlockParser\BlockQuoteParser(),    90)
            ->addBlockParser(new BlockParser\ATXHeadingParser(),    80)
            ->addBlockParser(new BlockParser\FencedCodeParser(),    70)
            ->addBlockParser(new BlockParser\HtmlBlockParser(),     60)
            ->addBlockParser(new BlockParser\SetExtHeadingParser(), 50)
            ->addBlockParser(new BlockParser\ThematicBreakParser(), 40)
            ->addBlockParser(new BlockParser\ListParser(),          30)
            ->addBlockParser(new BlockParser\IndentedCodeParser(),  20)
            ->addBlockParser(new BlockParser\LazyParagraphParser(), 10)

            ->addInlineParser(new InlineParser\NewlineParser(),     100)
            ->addInlineParser(new InlineParser\BacktickParser(),     90)
