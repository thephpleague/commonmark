...
/**
 * Provides an alternate case-folding mechanism for users without the mbstring library
 */
class UnicodeCaseFolder
{
    /**
     * @var array
     *
    ...
    /**
     * Converts the string to uppercase (using Unicode case folding rules)
     *
    ...
    {
        // Split into an array of Unicode characters
        $chars = preg_split('//u', $str, -1, PREG_SPLIT_NO_EMPTY);
        // Iterate through them, converting to uppercase where possible
        foreach ($chars as &$char) {
            $codePoint = static::ordutf8($char);
            // Several
            if ($key = array_search($codePoint, static::$map)) {
                $char = Html5Entities::fromDecimal($key);
            }
        }
    ...
    /**
     * Like ord(), but for UTF-8 chars
     *
     * Based on code found here: http://php.net/ord#109812
     *
     * @param string $char Unicode string character
     *
    ...
    protected static function ordutf8($char) {
        ...
        if ($code >= 128) {        //otherwise 0xxxxxxx
            ...
            for ($i = 2; $i <= $bytesnumber; $i++) {
                $code2 = ord(substr($char, 1, 1)) - 128;        //10xxxxxx
