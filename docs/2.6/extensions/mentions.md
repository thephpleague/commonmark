...
Whenever a mention is found, a `Mention` object is added to the [document's AST](/2.6/customization/abstract-syntax-tree/).
This object extends from `Link`, so it'll be rendered as a normal `<a>` tag by default.

If you need more control over the output you can implement a [custom renderer](/2.6/customization/rendering/) for the `Mention` type
