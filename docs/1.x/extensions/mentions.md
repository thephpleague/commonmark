...
redirect_from: /1.6/extensions/mentions/
---
...
Whenever a mention is found, a `Mention` object is added to the [document's AST](/1.x/customization/abstract-syntax-tree/).
This object extends from `Link`, so it'll be rendered as a normal `<a>` tag by default.

If you need more control over the output you can implement a [custom renderer](/1.x/customization/inline-rendering/) for the `Mention` type
