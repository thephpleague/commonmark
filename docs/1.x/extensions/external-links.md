...
redirect_from: /1.6/extensions/external-links/
---
...
When an external link is detected, the `ExternalLinkProcessor` will set the `external` data option on the `Link` node to either `true` or `false`.  You can therefore create a [custom link renderer](/1.x/customization/inline-rendering/) which checks this value and behaves accordingly:
