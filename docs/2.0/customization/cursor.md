...
Or, if you're creating a custom [block parser](/2.0/customization/block-parsing/) or [inline parser](/2.0/customization/inline-parsing/), a pre-configured `Cursor` will be provided to you with (with the `Cursor` already set to the current `position` trying to be parsed).
