...
- By calling the `getDocument()` method on either the `DocumentPreParsedEvent` or `DocumentParsedEvent` [see the (Event Dispatcher documentation](/2.7/customization/event-dispatcher/))

## Visualization

Even with an interactive debugger it can be tricky to view an entire tree at once.  Consider using the [`XmlRenderer`](/2.7/xml/) to provide a simple text-based representation of the AST for debugging purposes.
...
The best way to access and manipulate the AST is by adding an [event listener](/2.7/customization/event-dispatcher/) for the `DocumentParsedEvent`.
