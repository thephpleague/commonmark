...
The constructor methods for both `CommonMarkConverter` and `GithubFlavoredMarkdownConverter` no longer accept passing in a customized `Environment`.  If you want to customize the extensions used in your converter you should switch to using `MarkdownConverter`. See the [Basic Usage](/2.x/basic-usage/) documentation for an example.
...
Note that the `href` now targets the `id` attribute instead of the `name`, which is deprecated in HTML 5.  Additionally, the default prefix has changed to `content`.  See the [Heading Permalink extension documentation](/2.x/extensions/heading-permalinks/#configuration) for more details on how to configure the prefixes.
...
See [the block parsing documentation](/2.x/customization/block-parsing/) for more information on this new approach.
...
See [the inline parsing documentation](/2.x/customization/inline-parsing/) for more information on this new approach.
...
Need to generate unique slugs in your extensions? Use the new [Slug Normalizer](/2.x/customization/slug-normalizer/) provided by the `Environment`.
