...
redirect_from:
  - /customization/block-parsing/
  - /2.0/customization/block-parsing/
  - /2.1/customization/block-parsing/
  - /2.2/customization/block-parsing/
  - /2.3/customization/block-parsing/
  - /2.4/customization/block-parsing/
  - /2.5/customization/block-parsing/
  - /2.6/customization/block-parsing/
  - /2.7/customization/block-parsing/
---
...
Given a [`Cursor`](/2.x/customization/cursor/) at the current position, plus some extra information about the state of the parser, this method is responsible for determining whether a particular type of block seems to exist at the given position.  You don't actually parse the block here - that's the job of a `BlockContinueParserInterface`.  Your only job here is to return whether or not a particular type of block does exist here, and if so which block parser should parse it.
...
**NOTE:** If your custom block starts with a [letter character](http://unicode.org/reports/tr18/#General_Category_Property) you'll need to [add your parser to the environment](/2.x/customization/environment/#addblockstartparser) with a priority of `250` or higher.  This is due to a performance optimization where such lines are usually skipped.
